...
export const Render = (ndc, label) => {
  return {
    //------------NDC------------//
    DrugName: Capitalize(ndc?.data?.results[0]["generic_name"]) ?? "Data N/A",
    DosageForm: ndc?.data?.results[0]["dosage_form"] ?? "Data N/A",
    BrandName: Capitalize(ndc?.data?.results[0]["brand_name"]) ?? "Data N/A",
    ...
    DrugInteractions:
      label?.data?.results[0]["drug_interactions"] ?? "Data N/A",
    Contraindications:
      label?.data?.results[0]["contraindications"] ?? "Data N/A",
    InfoForPatients:
